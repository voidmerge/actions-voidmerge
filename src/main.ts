...
export async function showVersion(
  ...
  try {
    ...
    if (e instanceof Error) {
      ...
      throw new Error(`Error in showVersion: ${e}`)
    }
  }
}
...
export async function run(): Promise<any> {
  try {
    let toolVersion: string = core.getInput('voidmerge-version');
    ...
    core.info(`voidmerge version: ${installVersion}`);
    await installer(installVersion);
    result = await showVersion('vm', ['--version']);
